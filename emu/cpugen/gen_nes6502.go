...
func (g *Generator) opcodeHeader(code int) {
	...
	switch defs[code].m {
	case "imp":
		mode = `implied addressing.`
	case "acc":
		mode = `adressing accumulator.`
	case "rel":
		mode = `relative addressing.`
		g.mode = rel
	case "abs":
		mode = `absolute addressing.`
		g.mode = abs
	case "abx":
	...
	case "ind":
		...
		g.mode = ind
	case "izx":
		mode = `indexed addressing (abs, X).`
		g.mode = izx
	case "izy":
		mode = `indexed addressing (abs),Y.`
		g.mode = izy
	case "zpg":
		mode = `zero page addressing.`
		g.mode = zpg
	case "zpx":
		mode = `indexed addressing: zeropage,X.`
		g.mode = zpx
	case "zpy":
		mode = `indexed addressing: zeropage,Y.`
		g.mode = zpy
	}

	g.printf(`// %s - %s`, defs[code].n, mode)
