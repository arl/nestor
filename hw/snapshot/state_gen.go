...
func (z *NES) DecodeMsg(dc *msgp.Reader) (err error) {
	...
	for zb0001 > 0 {
		...
		case "RAM":
			...
			if cap(z.RAM) >= int(zb0002) {
				z.RAM = (z.RAM)[:zb0002]
			} else {
				z.RAM = make([]uint8, zb0002)
			}
			for za0001 := range z.RAM {
				z.RAM[za0001], err = dc.ReadUint8()
				if err != nil {
					err = msgp.WrapError(err, "RAM", za0001)
					return
				}
			}
		case "DMA":
...
func (z *NES) EncodeMsg(en *msgp.Writer) (err error) {
	...
	err = en.WriteArrayHeader(uint32(len(z.RAM)))
	if err != nil {
		err = msgp.WrapError(err, "RAM")
		return
	}
	for za0001 := range z.RAM {
		err = en.WriteUint8(z.RAM[za0001])
		if err != nil {
			err = msgp.WrapError(err, "RAM", za0001)
			return
		}
...
func (z *NES) Msgsize() (s int) {
	...
	s += 4 + msgp.ArrayHeaderSize + (len(z.RAM) * (msgp.Uint8Size)) + 4
	if z.DMA == nil {
		s += msgp.NilSize
	} else {
		s += z.DMA.Msgsize()
