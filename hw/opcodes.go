...
func opcode6B(cpu *CPU) {
	...
	cpu.A &= val
	cpu.A >>= 1
	cpu.P = cpu.P.SetV((cpu.A>>6)^(cpu.A>>5)&0x01 != 0)
	if cpu.P.C() {
		cpu.A |= 1 << 7
	}
	cpu.P.checkNZ(cpu.A)
	cpu.P = cpu.P.SetC(cpu.A&(1<<6) != 0)
}
